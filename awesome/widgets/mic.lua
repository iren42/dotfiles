...
-- command line: amixer -D pulse sset Capture mute
  function mic:mute()
    awful.spawn.easy_async({ "amixer","-D", "pulse","sset", "Capture", "mute" },
      function()
        self:update()
      end
    )
  ...
  function mic:unmute()
    awful.spawn.easy_async({ "amixer","-D", "pulse","sset", "Capture", "unmute" },
      function()
        self:update()
      end
    )
  ...
  -- command line: amixer -D pulse sget Capture
  ...
  mic, mic.timer = awful.widget.watch(
    { "bash", "-c", "amixer -D pulse sget Capture | grep '\\[on\\]'" },
    mic.timeout,
